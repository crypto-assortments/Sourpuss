SourPuss-qt: Qt4 GUI for SourPuss
===============================
...
Alternatively, install Qt Creator and open the `SourPuss-qt.pro` file.

An executable named `SourPuss-qt` will be built.
...
A warning for people using the *static binary* version of SourPuss on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SourPuss is linked against libdb5.3.
...
installed causes SourPuss-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
