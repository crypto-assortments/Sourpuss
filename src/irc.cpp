...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SourPussTEST2\r");
            Send(hSocket, "WHO #SourPussTEST2\r");
        } else {
            // randomly join #SourPuss00-#SourPuss05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SourPuss%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SourPuss%02d\r", channel_number).c_str());
        }
